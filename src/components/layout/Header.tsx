    ...
    <header className="h-20 bg-white border-b border-[#E6EFF5] px-8 flex items-center justify-between sticky top-0 z-20 shadow-[0_2px_10px_rgba(0,0,0,0.015)]">
      ...
      <div className="flex items-center gap-3 flex-1 max-w-lg">
        <button
          ...
          className="flex-1 flex items-center justify-between px-5 py-2.5 rounded-full bg-[#F5F7FA] hover:bg-[#EEF2F6] text-[#718EBF] transition-all text-sm group border border-[#E6EFF5]"
        >
          <div className="flex items-center gap-3">
            <Search className="w-4 h-4 text-[#718EBF] group-hover:text-[#2D60FF] transition-colors" />
            <span className="text-[#8BA3CB] font-normal truncate max-w-[200px] sm:max-w-none">
              Search Lot, Machine, Anomaly, Supplier...
            </span>
          </div>
        ...
        {/* Quick Copilot button in search cluster */}
        <button
          onClick={() => {
            sound.playClick();
            setIsCopilotOpen(true);
          }}
          className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-[#E7EDFF] hover:bg-[#d6e3ff] text-[#2D60FF] text-xs font-bold transition-all shadow-xs"
          title="Ask Manufacturing AI Copilot (Ctrl+J)"
        >
          <BrainCircuit className="w-4 h-4 animate-pulse text-[#2D60FF]" />
          <span className="hidden md:inline">AI Copilot</span>
        </button>
      </div>

      {/* Center Tagline Banner in BankDash pastel badge */}
      <div className="hidden 2xl:flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#E7EDFF] text-xs font-semibold text-[#2D60FF]">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <button
          ...
          className={`hidden sm:flex items-center gap-2 px-3 py-2 rounded-full text-xs font-medium transition-all ${
            isLiveTelemetryActive
        ...
        {/* 1-Click Preset Demonstration Scenarios Button */}
        <button
          onClick={() => {
            sound.playClick();
            setIsScenarioModalOpen(true);
          }}
          className="hidden md:flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-bold bg-[#FFF5D9] hover:bg-[#ffecc0] text-[#FFBB38] transition-all shadow-xs"
          title="Open 1-Click Story Incident Scenarios"
        >
        ...
        <button
          ...
          title="Launch 5-Minute Project Expo Story Tour (Ctrl+D)"
        >
        ...
        {/* Industry / Factory Selector with Dynamic Industrial Icon */}
        <div className="relative">
          <button
            onClick={() => setIsIndustryDropdownOpen(!isIndustryDropdownOpen)}
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#F5F7FA] hover:bg-[#EEF2F6] text-xs font-semibold text-[#343C6A] transition-colors border border-[#E6EFF5]"
            title={`Active Sector: ${industryConfig.name}`}
          >
            <IndustryVisualIcon type={currentIndustry} size={15} className="text-[#2D60FF]" />
          ...
          {isIndustryDropdownOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-3xl bg-white border border-[#E6EFF5] shadow-[0_10px_35px_rgba(0,0,0,0.08)] p-2 z-50">
              <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wider text-[#718EBF] border-b border-[#E6EFF5] flex items-center justify-between">
                <span>Select Industrial Sector</span>
                <span className="text-[10px] text-[#2D60FF] font-bold">Live Sector</span>
              </div>
                  ...
                  >
                    <span className="flex items-center gap-2.5 truncate">
                      <IndustryVisualIcon
                        type={ind.id}
                        size={16}
                        className={currentIndustry === ind.id ? 'text-[#2D60FF]' : 'text-[#718EBF]'}
                      />
